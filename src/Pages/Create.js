...
const Create = () => {
  ...
  const handleReset = (e) => {
    setFileExist(false);
    let newState = Object.assign(createData, { artType: undefined });
    setCreateData(newState);
    e.stopPropagation();
  };
  ...
  const handleResetPreview = (e) => {
    ...
    let newState = Object.assign(createData, { previewType: undefined });
    setCreateData(newState);
    e.stopPropagation();
  };

  const handleCreate = async () => {
    console.log('(createData.collectionType ',createData.collectionType)
    if(createData.artType==""){
      console.log('a')
    }
    if (createData.artType ===undefined ||createData.artType ===""|| (hasPreview && createData.previewType ===undefined)|| (hasPreview && createData.previewType ==="")||createData.name === undefined || createData.name ===""||createData.description === undefined || createData.description===""||createData.collectionType === undefined || createData.collectionType===""|| createData.price === undefined||createData.price === undefined) {
      if(createData.artType === undefined||createData.artType ==="") {
        setHasArt(false);
        document.getElementsByClassName(
          "ant-upload-drag"
      ...
      if (createData.name === undefined ||createData.name === "") {
        console.log(createData.name ,'createData.name ')
        setHasName(false);
     } 
     else{
      setHasName(true);
     ...
     if (createData.collectionType === undefined || createData.collectionType==="") {
        setHasCollection(false);
     } 
     else {
      setHasCollection(true);
     }
     if (createData.price === undefined||createData.price ==="") {
        setHasPrice(false);
     } 
     else{
      setHasPrice(true);
  ...
  const handleChangeCollection = (e) => {
    ...
    axios
      ...
      .then((res) => {
        console.log('collection api is called!',res)
        setList(res.data);
      })
      .catch((err) => {
        console.log('errsssssssssssssssssssssssss')
  ...
  const handleSelectChain = (name, index) => {
    var tmp = chains;
    tmp[index].name = tmp[0].name;
    tmp[0].name = name;
    setChains(tmp);
    setShowChain(false);
  };
                ...
                >
                  {list.length ? (
                    list.map((item, index) => {
                      return (
                        <div
                          className={`Inter18 sm-Inter9 font-grey-light mb-1 collection-field  ${
                            isDark
                              ? "back-dark grey-dark-border"
                              : "bg-white grey-border "
                          }`}
                    ...
                    <div
                      className={`Inter18 font-grey-light sm-Inter9  mb-1 collection-field  ${
                        isDark
                          ? "back-dark grey-dark-border"
                          : "bg-white grey-border "
                      }`}
        ...
        <section className="mt-30 mt-sm-20">
          ...
          <div className="mt-25 mt-sm-10">
            <Form.Item valuePropName="checked">
              <Switch defaultChecked={false} onChange={handleSetMintTyoe}/>
            </Form.Item>
          </div>
        </section>
        <section className="mt-30 mt-sm-20">
            ...
            {showChain && (
              ...
              >
                <div
                  ...
                  onClick={() => handleSelectChain(chains[1].name, 1)}
                >
                  <img
                    src={
                      chains[1].name == "Etherium" ? Etherium : BSC
                    }
                    className="chain-img"
                  />
